...
def water_jug_dfs(jug_x_capacity, jug_y_capacity, target_amount):
    visited = set()
    stack = [(State(0, 0), [])]  # Initialize stack with initial state and empty path
    ...
    while stack:
        ...
        if current_state.y > 0 and current_state.x < jug_x_capacity:
            ...
            if State(new_x, new_y) not in visited:
                stack.append((State(new_x, new_y), path + [current_state]))
...
def main():
    ...
    solution = water_jug_dfs(jug_x_capacity, jug_y_capacity, target_amount)
